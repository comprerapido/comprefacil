...
def build_homepage(input_path: str, template_path: str, output_path: str) -> None:
    ...
    # Garantir que os produtos têm um score, caso contrário, atribuir 0
    for p in products:
        if "score" not in p:
            p["score"] = 0
    ...
    # Selecionamos o produto com maior score para o destaque
    hero_product = sorted_products[0] if sorted_products else None
    
    if hero_product:
        logger.info(f"Destaque escolhido (mais recente): {hero_product.get('name')} (ID: {hero_product.get('id')})")
    else:
        logger.warning("Nenhum produto para destaque.")
        return
...
if __name__ == "__main__":
    build_homepage("data/scored_products.json", "templates/homepage.html", "index.html")
